...
void createutils_sh(const std::string & path, const params & p)
{
   ...
   std::string op;
   std::ofstream ofs;
   ofs.open(path+"/utils.sh");
   ofs << vdata;
   ofs.close();
   if (utils::bashcommand("chmod a+r "+path+"/utils.sh",op) != 0)
      logmsg(kLERROR, "Unable to change permissions on utils.sh",p);
   logmsg(kLDEBUG,"Created "+path+"/utils.sh",p);
}
