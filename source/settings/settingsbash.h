...
public:
   ...
   void setkeyvalue(const std::string &k, std::string & v); // raw value - includes quotes or whatever.

   std::string getline() const { return key + "=" + rawvalue; }

   bool valid() { return key.length() > 0; }

   const std::string & getkey() const { return key; }
   const std::string & getvalue() const { return rawvalue; }
   bashline(const bashline & other) { key = other.key; rawvalue = other.rawvalue; }
